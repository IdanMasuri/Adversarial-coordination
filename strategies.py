...
# numofvisibleconsensusnodes = ['1']
# concensusnodes = ['concensusnodes']
...
class Agent():
    ...
    def pickInitialColor(self, T1, T2, optimalchoose):
        """""
        ...
        if not optimalchoose:
            self.chooseColor(prob_pair, True)
        else:
            return prob_pair

    def pickSubsequentColor(self, T3, optimalchoose):
        """""
        ...
        if not optimalchoose:
            ...
            if self.currentColor == 'green':
                return {'red': prob_of_change, 'green': 1 - prob_of_change}
            else:
                return {'green': prob_of_change, 'red': 1 - prob_of_change}

    def optimalChoose(self, agnts, T1, T2, T3):
        agent_colors = dict()
        maxp = []
        for agent in agnts:
            if agent.type != 'vis':
                if agent.currentColor == 'white':
                    agent_colors[agent.unique_id] = agent.pickInitialColor(T1, T2, True)
                else:
                    agent_colors[agent.unique_id] = agent.pickSubsequentColor(T3, True)
                if agent_colors[agent.unique_id]['red'] == agent_colors[agent.unique_id]['green']:
                    maxp.append(random.choice(['green', 'red']))
                else:
                    maxp.append(max(agent_colors[agent.unique_id].items(), key=operator.itemgetter(1))[0])
        df = pd.DataFrame(agent_colors).T
        df['diff'] = abs(df['green'] - df['red'])
        df['maxp'] = maxp
        countmaxp = df['maxp'].value_counts()
        sum_diff_maxp = df.groupby('maxp')['diff'].sum()
        if len(countmaxp) > 1:
            rank = {'red': countmaxp['red'] * sum_diff_maxp['red'], 'green': countmaxp['green'] * sum_diff_maxp['green']}
            self.futureColor = max(rank.items(), key=operator.itemgetter(1))[0]

    def pickExpectedDominantColor(self, agnts, T1, T2, T3):
        agent_colors = {'green': 0, 'red': 0}
        for agent in agnts:
            if agent.type != 'vis':
                if agent.currentColor == 'white':
                    agent_colors[max(agent.pickInitialColor(T1, T2, True).items(), key=operator.itemgetter(1))[0]] += 1
                else:
                    agent_colors[max(agent.pickSubsequentColor(T3, True).items(), key=operator.itemgetter(1))[0]] += 1
        self.futureColor = max(agent_colors.items(), key=operator.itemgetter(1))[0]


def readConfigurations():
    configurations = pd.read_csv('OneVisConfigs.txt', header=None)
...
def addResults(agnts, confg, gtime, nc, T1, T2, T3):
    """""
    This function:
    ...
    for i in range(gtime):
        ...
        for agent in agnts:
            # if agent.type == 'vis' and confg[8] != 'nostrategy':
            #     agent.pickDominantColor(confg[8])
            if agent.type != 'vis':
                if agent.currentColor == 'white':
                    agent.pickInitialColor(T1, T2, False)
                else:
                    agent.pickSubsequentColor(T3, False)
            else:
                agent.pickExpectedDominantColor(agnts, T1, T2, T3)
...
def gameAgent(configs):
    ...
    for config in configs:
        game_time, simulations, m = 60, 1000, 3  # m : Initial edges for every node in the graph
        num_of_con_idx, num_of_vis_idx, num_of_adv_idx, network_idx, times_idx, con_rate_idx = 7, 5, 3, 0, 9, 11
        print(config)
        if config[num_of_con_idx] == '10' and int(config[num_of_adv_idx]) <= 5:
            max_degree = 7
        elif (config[num_of_con_idx] == '10' and int(config[num_of_adv_idx]) > 5) or (config[num_of_con_idx] == '20'):
            max_degree = 15
        else:
            max_degree = 21
        ...
        if config[network_idx] == 'barabasi-albert':
            for i in range(simulations):
                ...
                addResults(agents, config, game_time, num_of_con, CPM, RPM, CCM)
            ...
            for i in range(simulations):
                ...
                agents = createAgents(my_graph, num_of_adv, num_of_vis, num_of_reg)
                addResults(agents, config, game_time, num_of_con, CPM, RPM, CCM)
        config[con_rate_idx] = float(len(config[times_idx]) / simulations)
        config[times_idx] = np.mean(config[times_idx])
    return configs
...
# """User's input here:"""
# No_adj_circles = input(' Please enter number of adjacency circles as follows : \n1 : one circles\n2 : two circles'
#                        '\n3 : full recognition\n')

configurations = readConfigurations()
game_results = gameAgent(configurations)


"""Writing the results into txt file"""
with open('OptimalChooseRes2.txt', 'w') as configs:
